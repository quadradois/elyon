...
describe('AgendarFollowupUseCase', () => {
  ...
  it('propaga contrato estruturado e retorna o agregado duravel', async () => {
    ...
    const result = await new AgendarFollowupUseCase().execute({ tenantId: 't1', leadId: 'l1', dataRecontato: '01/01/2027 09:00', timezoneIana: 'America/Sao_Paulo', motivo: 'Retorno pedido', mensagemEnvio: 'Posso retomar nosso contato?', evidenciaPedido: 'pode me chamar dia 1', origemPedido: 'TOOL', requestId: 'request-create-1', policyVersion: 'followup-v1' });
  ...
  it('preserva reason code de esclarecimento sem persistir', async () => {
    ...
    await expect(new AgendarFollowupUseCase().execute({ tenantId: 't1', leadId: 'l1', dataRecontato: 'depois', timezoneIana: 'America/Sao_Paulo', motivo: 'pensar', mensagemEnvio: 'Posso retomar nosso contato?', evidenciaPedido: 'depois', origemPedido: 'TOOL', requestId: 'request-invalid-1' })).resolves.toMatchObject({ success: false, reasonCode: 'DATE_AMBIGUOUS' });
  });
  it('usa o caminho explicito de reagendamento quando recebe followupId', async () => {
    ...
    const result = await new AgendarFollowupUseCase().execute({ tenantId: 't1', leadId: 'l1', followupId: 'followup-ativo', dataRecontato: '02/01/2027 09:00', timezoneIana: 'America/Sao_Paulo', motivo: 'Retorno reagendado', mensagemEnvio: 'Novo retorno confirmado', evidenciaPedido: 'pode me chamar dia 2', origemPedido: 'TOOL_AGENDAR_FOLLOWUP', requestId: 'request-reschedule-1' });
