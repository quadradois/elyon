  ...
  try {
    ...
    if (campanha.status !== 'ATIVA') {
      return res.status(400).json({ 
        erro: `Campanha não está ativa (status: ${campanha.status})`,
    ...
    if (!ignorarValidacaoBriefing) {
      if (!campanha.briefingCompleto) {
        return res.status(400).json({
          erro: 'Campanha não possui briefing. Preencha o briefing do empreendimento antes de disparar.',
      ...
      if (!campanha.briefingValidado) {
        return res.status(400).json({
          erro: 'Briefing não foi validado. Revise e valide o briefing antes de disparar.',
    ...
    return res.status(500).json({ erro: error.message || 'Erro ao iniciar disparo' });
  }
});
...
router.post('/:id/pausar', async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: 'Erro ao pausar campanha' });
  }
});
...
router.post('/:id/reativar', async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: 'Erro ao reativar campanha' });
  }
});
...
router.get('/:id/status-disparo', async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: 'Erro ao buscar status do disparo' });
  }
});
...
router.get('/:id/config-disparo', async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: 'Erro ao buscar configurações de disparo' });
  }
});
...
router.put('/:id/config-disparo', async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: 'Erro ao salvar configurações de disparo' });
  }
});
...
router.get('/leads-quentes', async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: 'Erro ao buscar leads quentes' });
  }
});
...
router.get('/avaliacoes-agendadas', async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: 'Erro ao buscar avaliações agendadas' });
  }
});
