...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    return responderErro(res, 500, 'Erro ao buscar listas');
  }
});
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (!lista) {
      return responderErro(res, 404, 'Lista não encontrada');
    }
    ...
    return responderErro(res, 500, 'Erro ao buscar lista');
  }
});
  ...
  try {
    ...
    const lista = await prisma.lista.create({
      data: {
        tenantId,
        nome: dados.nome,
        nomeEdificio: dados.nomeEdificio || 'Não informado',
        localizacao: dados.localizacao || null,
        cep: dados.cep || null,
        dadosPesquisa: dados.dadosPesquisa || null,
    ...
    if (error.code === 'P2002') {
      return responderErro(res, 400, 'Contato duplicado na lista',
    ...
    if (error.code === 'P2003' || error.message?.includes('Foreign key constraint')) {
      ...
      return responderErro(res, 401, 'Sessão expirada',
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    return responderErro(res, 500, 'Erro ao excluir lista');
  }
});
...
router.post('/:id/adicionar-campanha', async (req: Request, res: Response) => {
  try {
    ...
    if (!campanhaId) {
      return responderErro(res, 400, 'campanhaId é obrigatório');
    }
    ...
    return responderErro(res, 500, 'Erro ao adicionar contatos à campanha');
  }
});
