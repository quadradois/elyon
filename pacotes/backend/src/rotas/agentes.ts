...
router.post('/configurar-rapido', async (req: Request, res: Response) => {
  try {
    ...
    responderErro(res, 500, 'Erro interno ao salvar perfil');
  }
});
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!agente) {
      return responderErro(res, 404, 'Agente não encontrado',
    ...
    responderErro(res, 500, 'Erro interno ao listar agente');
  }
});
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (tenantId && agente.tenantId !== tenantId) {
      return responderErro(res, 403, 'Acesso negado a este agente');
    }

    res.json({ agente });
  } catch (error) {
    console.error('[Agentes] Erro ao obter:', error);
    responderErro(res, 500, 'Erro interno ao obter agente');
  }
});
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!validacao.success) {
      ...
      return responderErro(res, 400, 'Dados inválidos',
    ...
    responderErro(res, 500, 'Erro interno ao criar agente');
  }
});
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    responderErro(res, 500, 'Erro interno ao atualizar agente');
  }
});
...
router.patch('/:id/ativar', async (req: Request, res: Response) => {
  try {
    ...
    if (errosValidacao.length > 0) {
      return responderErro(res, 400, 'Não é possível ativar o agente',
    ...
    responderErro(res, 500, 'Erro interno ao ativar agente');
  }
});
...
router.patch('/:id/pausar', async (req: Request, res: Response) => {
  try {
    ...
    if (agenteExistente.status !== 'ATIVO') {
      return responderErro(res, 400, 'Só é possível pausar agentes ativos',
    ...
    responderErro(res, 500, 'Erro interno ao pausar agente');
  }
});
...
router.patch('/:id/desativar', async (req: Request, res: Response) => {
  try {
    ...
    responderErro(res, 500, 'Erro interno ao desativar agente');
  }
});
...
router.patch('/:id/aceitar-termos', async (req: Request, res: Response) => {
  try {
    ...
    responderErro(res, 500, 'Erro interno ao aceitar termos');
  }
});
...
router.patch('/:id/perfil-imobiliaria', async (req: Request, res: Response) => {
  try {
    ...
    if (!validacao.success) {
      return responderErro(res, 400, 'Dados do perfil inválidos',
        {detalhes: validacao.error.flatten()});
    }
    ...
    responderErro(res, 500, 'Erro interno ao atualizar perfil');
  }
});
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (agenteExistente.estaAtivo) {
      return responderErro(res, 400, 'Não é possível excluir um agente ativo',
    ...
    responderErro(res, 500, 'Erro interno ao excluir agente');
  }
});
...
router.patch('/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    responderErro(res, 500, 'Erro interno ao mudar status');
  }
});
...
router.post('/modo-avancado', async (req: Request, res: Response) => {
  try {
    ...
    if (agenteExistente) {
      return responderErro(res, 409, 'Agente já existe',
    ...
    if (!validacao.success) {
      return responderErro(res, 400, 'Dados inválidos para modo avançado',
        {detalhes: validacao.error.flatten()});
    }
    ...
    responderErro(res, 500, 'Erro interno ao criar agente avançado');
  }
});
