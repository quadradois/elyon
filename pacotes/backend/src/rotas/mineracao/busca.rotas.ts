...
router.get('/bairros', async (req, res) => {
  try {
    ...
    console.log(`[Mineracao] Listando bairros${termo ? ` com termo "${termo}"` : ''}...`);
    const bairros = await mapaService.listarBairros(termo);
    ...
    return responderErro(res, 500, 'Erro ao listar bairros');
  }
});
...
router.get('/edificios/:cdbairro', async (req, res) => {
  try {
    ...
    if (isNaN(cdbairro)) {
      return responderErro(res, 400, 'Código do bairro inválido');
    }
    ...
    return responderErro(res, 500, 'Erro ao listar edifícios');
  }
});
...
router.get('/unidades/:cdedificio', async (req, res) => {
  try {
    ...
    if (isNaN(cdedificio)) {
      return responderErro(res, 400, 'Código do edifício inválido');
    }
    ...
    return responderErro(res, 500, 'Erro ao buscar unidades');
  }
});
...
router.get('/buscar-edificios', async (req, res) => {
  try {
    ...
    if (!termo || termo.length < 2) {
      return responderErro(res, 400, 'Termo de busca deve ter pelo menos 2 caracteres');
    }
    ...
    return responderErro(res, 500, 'Erro ao buscar edifícios');
  }
});
  ...
  try {
    ...
    if (codigo !== undefined) {
      ...
      if (!/^\d+$/.test(codigoLimpo) || codigoLimpo.length < 3) {
        return responderErro(res, 400, 'Código inválido. Informe apenas números (mínimo 3 dígitos)');
      }
    ...
    return responderErro(res, 500, 'Erro ao buscar imóveis');
  }
});
...
router.get('/condominios', async (req, res) => {
  try {
    ...
    return responderErro(res, 500, 'Erro ao buscar condomínios');
  }
});
...
router.get('/casas/:cdbairro', async (req, res) => {
  try {
    ...
    if (isNaN(cdbairro)) {
      return responderErro(res, 400, 'Código do condomínio inválido');
    }
    ...
    return responderErro(res, 500, 'Erro ao listar casas');
  }
});
...
router.get('/endereco', async (req, res) => {
  try {
    ...
    if (!rua || rua.length < 3) {
      return responderErro(res, 400, 'Nome da rua deve ter pelo menos 3 caracteres');
    }
    ...
    return responderErro(res, 500, 'Erro ao buscar por endereço');
  }
});
