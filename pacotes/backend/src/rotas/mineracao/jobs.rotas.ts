...
router.post('/iniciar', async (req: Request, res: Response) => {
    try {
        ...
        return res.status(500).json({ erro: 'Erro ao iniciar job de mineração' });
    }
});
...
router.get('/:id/status', async (req: Request, res: Response) => {
    try {
        ...
        return res.status(500).json({ erro: 'Erro ao obter status do job' });
    }
});
...
router.get('/:id/resultado', async (req: Request, res: Response) => {
    try {
        ...
        return res.status(500).json({ erro: 'Erro ao obter resultado do job' });
    }
});
...
router.delete('/:id', async (req: Request, res: Response) => {
    try {
        ...
        if (!cancelado) {
            return res.status(400).json({
                erro: 'Não foi possível cancelar o job (já concluído ou não existe)'
            });
        }
        ...
        return res.status(500).json({ erro: 'Erro ao cancelar job' });
    }
});
