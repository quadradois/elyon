...
// ✅ HELPER SEGURO - sem fallback inseguro
  ...
  try {
    ...
    if (!manusService.estaConfigurado()) {
      return res.status(503).json({ 
        erro: 'Serviço de pesquisa não configurado',
    ...
    return res.status(500).json({ erro: 'Erro ao iniciar pesquisa', detalhes: error.message });
  }
});
...
router.get('/', async (req, res) => {
  try {
    ...
    console.error('[PESQUISA] Erro:', error);
    return res.status(500).json({ erro: 'Erro ao listar pesquisas' });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!pesquisa) {
      return res.status(404).json({ erro: 'Pesquisa não encontrada' });
    }
    ...
    console.error('[PESQUISA] Erro:', error);
    return res.status(500).json({ erro: 'Erro ao buscar pesquisa' });
  }
});
...
router.post('/:id/verificar', async (req, res) => {
  try {
    ...
    console.error('[PESQUISA] Erro:', error);
    return res.status(500).json({ erro: 'Erro ao verificar pesquisa', detalhes: error.message });
  }
});
...
router.post('/:id/aplicar', async (req, res) => {
  try {
    ...
    if (pesquisa.status !== 'CONCLUIDO') {
      return res.status(400).json({ erro: 'Pesquisa ainda não foi concluída' });
    }

    if (!pesquisa.resultado) {
      return res.status(400).json({ erro: 'Pesquisa não possui resultado' });
    }
    ...
    console.error('[PESQUISA] Erro:', error);
    return res.status(500).json({ erro: 'Erro ao aplicar RAG', detalhes: error.message });
  }
});
...
router.post('/webhook/registrar', async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: 'Erro ao registrar webhook', detalhes: error.message });
  }
});
...
router.delete('/webhook/:webhookId', async (req, res) => {
  try {
    ...
    if (!manusService.estaConfigurado()) {
      return res.status(503).json({ erro: 'Serviço de pesquisa não configurado' });
    }
    ...
    return res.status(500).json({ erro: 'Erro ao remover webhook', detalhes: error.message });
  }
});
