...
describe('construirInputSdk', () => {
  ...
  it('monta input de primeiro turno com system + histórico em formato da Responses API', () => {
    ...
    expect(result.inputSDK[0].role).toBe('system');
    expect(result.inputSDK[0].content).toContain('CONTEXTO DO LEAD');
    expect(result.inputSDK[0].content).toContain('NOSSO MÉTODO DE TRABALHO');
    expect(result.inputSDK[0].content).toContain('comissão segue a política comercial vigente');
  ...
  it('usa ramo de cache e anexa resumo + última mensagem do usuário', () => {
    ...
    const cachedHistory = [{ role: 'system', content: 'history-item' }];
    ...
    expect(result.inputSDK[1].role).toBe('system');
    expect(result.inputSDK[1].content).toContain('ESTADO RESUMIDO');
  ...
  it('inclui resumo de schemaState quando informado', () => {
    const result = construirInputSdk({
      ...
      estadoConversaAtual: estadoBase,
      schemaState: { intencao: 'vender', metragem: 92, ocupacao: 'ocupado' },
      config: {},
      contexto: {},
    });

    expect(result.inputSDK[0].content).toContain('HISTÓRICO DE COLETA');
    expect(result.inputSDK[0].content).toContain('intencao');
  });
  ...
  it('inclui leadRecord quando presente', () => {
    ...
    expect(result.inputSDK[0].content).toContain('DADOS EXISTENTES NO LEAD');
    expect(result.inputSDK[0].content).toContain('nome: João');
  });
  ...
  it('prioriza ragPerfilTexto e adiciona briefing do empreendimento no primeiro turno', () => {
    ...
    expect(result.origem).toBe('primeiro_turno');
    expect(result.inputSDK[0].content).toContain('PERFIL COMPLETO DA IMOBILIÁRIA');
    expect(result.inputSDK[0].content).toContain('Somos especialistas no bairro X.');
    expect(result.inputSDK[0].content).toContain('CONHECIMENTO DO EMPREENDIMENTO: Residencial Jardim');
    expect(result.inputSDK[0].content).toContain('Torre única com lazer completo.');
  });
});
