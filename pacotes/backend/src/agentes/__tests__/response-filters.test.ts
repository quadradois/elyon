...
describe('aplicarFiltrosRespostaOrchestrator', () => {
  ...
  it('aciona gate runtime quando presenter tenta mover fase sem qualificar_lead no turno', () => {
    const result = aplicarFiltrosRespostaOrchestrator({
      respostaFinal: 'Perfeito. Fechamos e seguimos para documentação.',
      houveHandoff: false,
      tipoAgente: 'PRESENTER',
      agenteQueRespondeuFormatado: 'PRESENTER',
      estadoConversaAtual: estadoBase,
    ...
    expect(result.fallbackAplicado).toBe('RUNTIME_SPIN_TOOL_GATE');
    expect(result.respostaLimpa).toContain('fechar seu diagnóstico com precisão');
  });

  it('aciona gate runtime quando opener tenta transição sem converter/qualificar', () => {
    const estadoSemValor = {
      ...estadoBase,
      valorPretendido: null,
      // preserve new flags
      estaAnunciando: estadoBase.estaAnunciando,
      timeline: estadoBase.timeline,
    };

    const result = aplicarFiltrosRespostaOrchestrator({
      respostaFinal: 'Perfeito, peguei os dados principais. Posso te fazer uma pergunta rápida sobre sua prioridade agora?',
      houveHandoff: false,
      tipoAgente: 'OPENER',
      agenteQueRespondeuFormatado: 'OPENER',
      estadoConversaAtual: estadoSemValor as any,
      cotLog: null,
      nomesToolsTurno: [],
      fallbackAplicadoAtual: 'NONE',
    });

    expect(result.fallbackAplicado).toBe('OPENER_CONVERSION_GATE');
    expect(result.respostaLimpa).toContain('valor de venda em mente');
  });
});
