...
export function filterHistoryByQuery(
    ...
    if (removidos.length > 0) {
        console.log(`[HANDOFF-FILTER] 📊 ${context || 'Filtro'}: ${antes} → ${filtered.length} itens (${removidos.length} removidos)`);
        removidos.forEach(r => console.log(`  ❌ ${r}`));
    }
...
export function sliceHistoryPreservingSystem(
    ...
    // Preservar também mensagens de tool call que podem conter dados importantes do lead
    const toolCallItems = history.filter(item => {
        const itemAny = item as any;
        return itemAny.type === 'tool_call_item' || itemAny.type === 'tool_call_output_item';
    });
    ...
    if (removidos > 0) {
        console.log(`[HANDOFF-FILTER] 📊 ${context || 'Slice'}: ${nonSystemItems.length} → ${strategyItems.length} turnos (${removidos} antigos descartados, ${systemItems.length} system + ${toolCallItems.length} tool calls preservados)`);
    }

    return [...systemItems, ...toolCallItems, ...strategyItems];
}
 ...
 * Usa gpt-4.1-mini para custo mínimo (~$0.003 por chamada).
 */
...
Seja DIRETO e TÁTICO. Não use introduções. Comece direto com os fatos.`
        ...
        console.log(`[HANDOFF-FILTER] 🧠 Briefing LLM gerado (${agenteOrigem} → ${agenteDestino}): ${briefing.substring(0, 80)}...`);
        ...
        console.warn(`[HANDOFF-FILTER] ⚠️ Erro ao gerar briefing LLM (não-crítico):`, err);
        return null;
    }
}
