        ...
        <div className="p-4 border-b border-gray-200 bg-gray-50/50 flex flex-col md:flex-row gap-4">
          ...
          <div className="relative w-full md:w-64">
            ...
            <select
              ...
              className="w-full pl-9 pr-8 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 appearance-none"
            >
                  ...
                  <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-gray-900 border border-gray-200 bg-white px-2 py-0.5 rounded-md text-xs">
                          {log.acao}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {log.usuario ? (
                        <div>
                          <p className="font-medium text-gray-900 truncate max-w-[150px]">{log.usuario.nome}</p>
                          <p className="text-xs text-gray-500 truncate max-w-[150px]">{log.tenant?.nome || 'Global'}</p>
                        </div>
                      ) : (
                        <span className="text-gray-400 italic">Sistema/Anon</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-xs text-gray-600 bg-gray-50 rounded p-2 border border-gray-100 font-mono max-h-20 overflow-y-auto w-64 md:w-auto">
        ...
        {paginacao.totalPaginas > 1 && (
          <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between text-sm">
            <span className="text-gray-500">
              Mostrando página <span className="font-medium text-gray-900">{paginacao.pagina}</span> de <span className="font-medium text-gray-900">{paginacao.totalPaginas}</span>
            </span>
