 ...
 * 
 * Design: Sistema de abas para separar Atendimento, Proprietário e Imóvel
 * Chat lateral sempre visível para comunicação rápida
 */
                    ...
                    return (
                      ...
                      >
                        <div className={`max-w-[85%]`}>
                          <div
                            className={`rounded-2xl px-4 py-2.5 ${isSaida
              ...
              <div className="p-4 border-t border-slate-100">
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    className="bg-blue-600 hover:bg-blue-700 h-[44px] w-[44px] p-0"
                  >
                    {enviando ? (
    ...
    <div className="space-y-6">
        ...
        <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
          ...
          <div className="flex items-center gap-1 bg-white rounded-lg p-1 border border-slate-200">
            <Button
              ...
              className={`flex-1 h-9 ${contato.modoAtendimento === 'IA' || !contato.modoAtendimento ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'text-slate-600 hover:text-slate-900'}`}
            >
            ...
            <Button
              ...
              className={`flex-1 h-9 ${contato.modoAtendimento === 'HUMANO' ? 'bg-amber-600 hover:bg-amber-700 text-white' : 'text-slate-600 hover:text-slate-900'}`}
            >
            ...
            <Button
              ...
              className={`flex-1 h-9 ${contato.modoAtendimento === 'PAUSADO' ? 'bg-slate-600 hover:bg-slate-700 text-white' : 'text-slate-600 hover:text-slate-900'}`}
            >
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-5 border border-blue-100">
            ...
            <div className="h-2 bg-blue-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min(100, contato.scoreAssertiva)}%` }}
              />
            </div>
            <p className="text-xs text-blue-600 mt-2">
              {contato.scoreAssertiva >= 80 ? '✓ Dados confiáveis' :
                contato.scoreAssertiva >= 50 ? '⚠ Verificar dados' : '✗ Dados incompletos'}
            </p>
          </div>
        )}
      ...
      <div className="bg-amber-50 rounded-xl p-5 border border-amber-100">
        ...
        <p className="text-sm text-amber-800 leading-relaxed whitespace-pre-wrap">
          {contato.observacoes || 'Nenhuma observação registrada.'}
        </p>
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            ...
            {contato.dataNascimento && (
              <div className="flex justify-between py-2 border-b border-slate-200">
                ...
                <span className="text-sm text-slate-900">{contato.dataNascimento}</span>
              </div>
            )}
            ...
            {contato.situacaoCadastral && (
              <div className="flex justify-between py-2">
                <span className="text-sm text-slate-500">Situação</span>
                <span className={`text-sm font-medium ${contato.situacaoCadastral === 'REGULAR' ? 'text-emerald-600' : 'text-amber-600'}`}>
                  {contato.situacaoCadastral}
                </span>
              </div>
            )}
        ...
        <div className="bg-blue-50 rounded-xl p-5 border border-blue-100">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-blue-900">Dados Profissionais</h3>
          </div>

          <div className="space-y-3">
            {contato.empresaAtual && (
              <div>
                <p className="text-xs text-blue-500 uppercase mb-1">Empresa</p>
                <p className="text-sm font-medium text-blue-900">{contato.empresaAtual}</p>
              </div>
            )}

            {contato.profissao && (
              <div>
                <p className="text-xs text-blue-500 uppercase mb-1">Cargo/Profissão</p>
                <p className="text-sm text-blue-800">{contato.profissao}</p>
              </div>
            )}

            {contato.setor && (
              <div>
                <p className="text-xs text-blue-500 uppercase mb-1">Setor</p>
                <p className="text-sm text-blue-800">{contato.setor}</p>
              </div>
            )}
            ...
            {contato.rendaEstimada && (
              <div className="bg-gradient-to-r from-emerald-50 to-green-50 rounded-lg p-3 mt-3 border border-emerald-200">
                ...
                <p className="text-xl font-bold text-emerald-700">
                  {formatarMoeda(contato.rendaEstimada)}
                </p>
                {contato.faixaSalarial && (
                  <p className="text-xs text-emerald-600 mt-1">{contato.faixaSalarial}</p>
                )}
              </div>
            )}

            {!contato.empresaAtual && !contato.profissao && !contato.rendaEstimada && (
              <p className="text-sm text-blue-400 italic">Dados profissionais não disponíveis</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="bg-emerald-50 rounded-xl p-5 border border-emerald-100">
          ...
          <div className="space-y-2">
            {telefones.slice(0, 5).map((tel: any, idx: number) => (
              <div
                key={idx}
                className="group flex items-center justify-between p-2.5 rounded-lg bg-white border border-emerald-200 hover:border-emerald-300 transition-colors"
              >
                <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-emerald-100 transition-all"
                >
            ...
            {telefones.length === 0 && (
              <p className="text-sm text-emerald-400 italic">Nenhum telefone cadastrado</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-violet-50 rounded-xl p-5 border border-violet-100">
          ...
          <div className="space-y-2">
            {emails.filter((e: any) => e?.email).slice(0, 5).map((email: any, idx: number) => (
              <div
                key={idx}
                className="group flex items-center justify-between p-2.5 rounded-lg bg-white border border-violet-200 hover:border-violet-300 transition-colors"
              >
            ...
            {emails.filter((e: any) => e?.email).length === 0 && (
              <p className="text-sm text-violet-400 italic">Nenhum email cadastrado</p>
            )}
          </div>
        </div>
      </div>
      ...
      {(contato.endereco || contato.cidade) && (
        <div className="bg-amber-50 rounded-xl p-5 border border-amber-100">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-amber-900">Endereço Pessoal</h3>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              {contato.cidade && contato.estado && (
                <p className="text-sm text-slate-600 mt-0.5">{contato.cidade} - {contato.estado}</p>
              )}
              {contato.cep && (
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-slate-400 max-w-sm">
          As informações do imóvel serão exibidas aqui quando disponíveis.
        </p>
      </div>
    );
  }
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            {contato.nomeEdificio && (
              <h3 className="text-xl font-bold text-slate-900 mb-1">{contato.nomeEdificio}</h3>
            )}
            {contato.enderecoImovel && (
              <p className="text-sm text-slate-600">{contato.enderecoImovel}</p>
            )}
            {contato.bairroImovel && (
              <p className="text-sm text-slate-500">{contato.bairroImovel}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-4">
          {contato.apartamento && (
            <div className="bg-white rounded-lg px-4 py-2 border border-blue-200">
              <p className="text-[10px] text-blue-500 uppercase">Apartamento</p>
              <p className="text-lg font-bold text-blue-700">{contato.apartamento}</p>
            </div>
          )}
          {contato.bloco && (
            <div className="bg-white rounded-lg px-4 py-2 border border-blue-200">
              <p className="text-[10px] text-blue-500 uppercase">Bloco</p>
              <p className="text-lg font-bold text-blue-700">{contato.bloco}</p>
            </div>
          )}
          {contato.unidade && !contato.apartamento && (
            <div className="bg-white rounded-lg px-4 py-2 border border-blue-200">
              <p className="text-[10px] text-blue-500 uppercase">Unidade</p>
              <p className="text-lg font-bold text-blue-700">{contato.unidade}</p>
            </div>
          )}
          {contato.box && (
            <div className="bg-white rounded-lg px-4 py-2 border border-slate-200">
              <p className="text-[10px] text-slate-500 uppercase">Box/Garagem</p>
              <p className="text-lg font-bold text-slate-700">{contato.box}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-slate-700">Características</h3>
          </div>
          ...
          <div className="space-y-3">
            {contato.tipoImovel && (
              <div className="flex justify-between py-2 border-b border-slate-200">
                ...
                <span className="text-sm font-medium text-slate-900">{contato.tipoImovel}</span>
              </div>
            )}
            {contato.areaTerreno && (
              <div className="flex justify-between py-2 border-b border-slate-200">
                <span className="text-sm text-slate-500">Área Terreno</span>
                <span className="text-sm font-medium text-slate-900">{contato.areaTerreno} m²</span>
              </div>
            )}
            {contato.areaConstruida && (
              <div className="flex justify-between py-2 border-b border-slate-200">
                <span className="text-sm text-slate-500">Área Construída</span>
                <span className="text-sm font-medium text-slate-900">{contato.areaConstruida} m²</span>
              </div>
            )}
            {contato.valorVenal && (
              <div className="flex justify-between py-2">
                ...
                <span className="text-sm font-medium text-emerald-600">
                  {formatarMoeda(contato.valorVenal)}
                </span>
              </div>
            )}
            {!contato.tipoImovel && !contato.areaTerreno && !contato.valorVenal && (
              <p className="text-sm text-slate-400 italic">Características não disponíveis</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-slate-700">Localização</h3>
          </div>

          <div className="space-y-3">
            {contato.quadra && (
              <div className="flex justify-between py-2 border-b border-slate-200">
                <span className="text-sm text-slate-500">Quadra</span>
                <span className="text-sm font-medium text-slate-900">{contato.quadra}</span>
              </div>
            )}
            {contato.lote && (
              <div className="flex justify-between py-2 border-b border-slate-200">
                <span className="text-sm text-slate-500">Lote</span>
                <span className="text-sm font-medium text-slate-900">{contato.lote}</span>
              </div>
            )}
            {contato.bairroImovel && (
              <div className="flex justify-between py-2">
                <span className="text-sm text-slate-500">Bairro</span>
                <span className="text-sm font-medium text-slate-900">{contato.bairroImovel}</span>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="font-semibold text-slate-700">Inscrição IPTU</h3>
              </div>
              <p className="font-mono text-lg font-medium text-slate-900">{contato.inscricaoIptu}</p>
            </div>
            <button
              onClick={() => copiar(contato.inscricaoIptu!, 'IPTU')}
              className="p-2.5 rounded-lg bg-white border border-slate-200 hover:bg-slate-50 transition-colors"
            >
              {copiado === 'IPTU' ? <Check className="w-5 h-5 text-emerald-500" /> : <Copy className="w-5 h-5 text-slate-400" />}
            </button>
          </div>
        </div>
      )}
