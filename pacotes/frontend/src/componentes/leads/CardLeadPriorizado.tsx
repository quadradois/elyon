      ...
      <div className="p-4">
        ...
        {(lead.campanhaOrigem || lead.scoreComposto != null) && (
          <div className="flex items-center justify-between mb-2.5">
            {lead.campanhaOrigem ? (
              <div className="flex items-center gap-1.5 text-[11px] text-indigo-600 font-medium bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-100">
        ...
        {/* Linha 5: Imóvel + Assertiva */}
        {(lead.tipoImovel || lead.valorPretendido || lead.bairroImovel || lead.scoreAssertiva || lead.rendaEstimada || lead.faixaSalarial) && (
          <div className="flex items-stretch gap-2 mb-3">
            {(lead.tipoImovel || lead.valorPretendido || lead.bairroImovel) && (
              <div className="flex items-start gap-1.5 flex-1 min-w-0 text-[11px] text-slate-600 bg-slate-50 px-2.5 py-2 rounded-xl border border-slate-100">
                <Home className="w-3 h-3 mt-0.5 flex-shrink-0 text-slate-400" />
                <span className="truncate">
                  {[lead.tipoImovel, lead.valorPretendido, lead.bairroImovel].filter(Boolean).join(' · ')}
                </span>
              </div>
            )}
